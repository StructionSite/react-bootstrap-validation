...
export default class Form extends InputContainer {
  ...
  registerInput(input) {
    ...
    if (typeof input.props.validate === "string") {
      this._validators[input.props.name] = this._compileValidationRules(
        input,
  ...
  _renderChild(child) {
    ...
    if (
      ...
      return React.cloneElement(
        child,
        {},
        this._renderChildren(child.props && child.props.children)
      );
    }
  }
  ...
  _setError(iptName, isError, errText) {
    ...
    this.setState({
      invalidInputs: Object.assign(this.state.invalidInputs, {
  ...
  _compileValidationRules(input, ruleProp) {
    ...
    let validator =
      (input.props && input.props.type) === "file" ? FileValidator : Validator;
    ...
    return val => {
      ...
      rules.forEach(rule => {
        ...
        if (result === true && ruleResult !== true) {
          result =
            getInputErrorMessage(input, rule.name) ||
            getInputErrorMessage(this, rule.name) ||
            false;
        }
      });
...
Form.propTypes = {
  ...
  validationEvent: PropTypes.oneOf(["onChange", "onBlur", "onFocus"]),
  errorHelp: PropTypes.oneOfType([PropTypes.string, PropTypes.object])
};
